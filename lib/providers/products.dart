import 'package:flutter/cupertino.dart';
import 'package:http/http.dart' as http;
import 'package:flutter/material.dart';
import 'product.dart';
import 'dart:convert';

class Products with ChangeNotifier {
  // List<Product> _items = [
  //   Product(
  //     id: 'p1',
  //     title: 'Red Shirt',
  //     description: 'A red shirt - it is pretty red!',
  //     price: 29.99,
  //     imageURL:
  //         'https://cdn.pixabay.com/photo/2016/10/02/22/17/red-t-shirt-1710578_1280.jpg',
  //   ),
  //   Product(
  //     id: 'p2',
  //     title: 'Trousers',
  //     description: 'A nice pair of trousers.',
  //     price: 59.99,
  //     imageURL:
  //         'https://upload.wikimedia.org/wikipedia/commons/thumb/e/e8/Trousers%2C_dress_%28AM_1960.022-8%29.jpg/512px-Trousers%2C_dress_%28AM_1960.022-8%29.jpg',
  //   ),
  //   Product(
  //     id: 'p3',
  //     title: 'Yellow Scarf',
  //     description: 'Warm and cozy - exactly what you need for the winter.',
  //     price: 19.99,
  //     imageURL:
  //         'https://live.staticflickr.com/4043/4438260868_cc79b3369d_z.jpg',
  //   ),
  //   Product(
  //     id: 'p4',
  //     title: 'A Pan',
  //     description: 'Prepare any meal you want.',
  //     price: 49.99,
  //     imageURL:
  //         'https://upload.wikimedia.org/wikipedia/commons/thumb/1/14/Cast-Iron-Pan.jpg/1024px-Cast-Iron-Pan.jpg',
  //   ),
  // ];
  final String? _authToken;
  List<Product> _items = [];
  Products(this._authToken, this._items);
  List<Product> get items {
    return [..._items];
  }

  List<Product> get favitems {
    return _items!.where((element) => element.isFavourite).toList();
  }

  Future<void> fetchandget() async {
    final urli =
        'https://flutter-ca6b2-default-rtdb.firebaseio.com/products.json?auth=$_authToken';
    try {
      final response = await http.get(Uri.parse(urli));
      print(json.decode(response.body));
      final extractedProds = json.decode(response.body) as Map<String, dynamic>;
      List<Product> loadedProducts = [];
      extractedProds.forEach((key, value) {
        loadedProducts.add(Product(
            id: key.toString(),
            title: value['title'],
            price: value['price'],
            description: value['description'],
            imageURL: value['imageURl'],
            isFavourite: value['isFavoutite']));
      });
      _items = loadedProducts;
      notifyListeners();
    } catch (error) {
      print('bhak');
      throw error;
    }
  }

  Future<void> addProducts(Product value) async {
    const urli = 'https://flutter-ca6b2-default-rtdb.firebaseio./';
    final newProduct = Product(
        description: value.description,
        id: DateTime.now().toString(),
        price: value.price,
        imageURL: value.imageURL,
        title: value.title);
    try {
      http.post(Uri.parse(urli),
          body: json.encode({
            'title': value.title,
            'description': value.description,
            'id': value.id,
            'price': value.price,
            'imageURl': value.imageURL,
            'isFavoutite': value.isFavourite
          }));
      _items!.add(newProduct);
      notifyListeners();
    } catch (error) {
      throw error;
    }
  }

  Product findById(String prodID) {
    return items.firstWhere((prod) => prod.id == prodID);
  }
}
